...
def train(criterion: Callable, model: I2GNN, device: torch.device,
          loader: DataLoader, optimizer: optim.Optimizer, task_type: str):
    model.train()
    losss = []
    for batch in loader:
...
def parserarg():
    ...
    parser.add_argument('--model', type=str, default="policygrad")
...
def buildModel(args, num_tasks, device, dataset):
    ...
    model = I2GNN(dataset, num_tasks, args.num_layer, args.emb_dim, args.gpool,
                  args.lpool, args.res, args.outlayer, args.ln_out,
                  **kwargs).to(device)
...
def main():
    ...
    for rep in range(args.repeat):
        ...
        schedulerwst = optim.lr_scheduler.LambdaLR(
            optimizer, lambda epoch: 0.9**(args.warmstart - epoch))
        schedulerdc = optim.lr_scheduler.LambdaLR(
            optimizer, lambda epoch: 1 / (1 + epoch *
                                          (args.K + args.K2 * epoch)))
        scheduler = optim.lr_scheduler.SequentialLR(
            optimizer,
            schedulers=[schedulerwst, schedulerdc],
            milestones=[args.warmstart],
            verbose=True)
        normscd = NormMomentumScheduler(
            lambda x: 1 / (1 + x * (args.normK + x * args.normK2)),
            args.normparam, normdict[args.nnnorm])
        valid_curve = []
        test_curve = []
        train_curve = []
        ...
        for epoch in range(1, args.epochs + 1):
            t1 = time.time()
            loss = train(get_criterion(task, args), model, device,
                         train_loader, optimizer, task)
            print(
