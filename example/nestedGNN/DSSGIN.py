...
from subgnn.Spconv import Convs, SUNConv
...
class DSSGIN(nn.Module):
    ...
    def __init__(self,
        ...
        self.subggnns = Convs(
            [SUNConv(emb_dim, **kwargs["conv"]) for i in range(num_layer)],
            residual=residual)
    ...
    def forward(self, datadict: dict):
        ...
        x = pooling2nodes(X, dim=1, pool=self.lpool)
