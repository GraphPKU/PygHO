...
class SSWLConv(Module):
    ...
    def __init__(self,
        ...
        self.lin = MLP(3*emb_dim, emb_dim, mlplayer, True, **mlp)
...
class DSSGNNConv(Module):
    ...
    def __init__(self,
        ...
        self.lin = MLP(2*emb_dim, emb_dim, mlplayer, True, **mlp)
...
class PPGNConv(Module):
    ...
    def forward(self, X: Union[SparseTensor, MaskedTensor], datadict: dict) -> Union[SparseTensor, MaskedTensor]:
        return self.op.forward(X.tuplewiseapply(self.lin1), X.tuplewiseapply(self.lin2), datadict, X)
...
class GNNAKConv(Module):
    def __init__(self,
                 ...
                 ctx: bool=True):
        super().__init__()
        self.lin0 = MLP(emb_dim, emb_dim, mlplayer, True, **mlp)
        self.aggr = TensorOp.OpMessagePassingOnSubg2D(mode, aggr)
        ...
        self.lin = MLP(3*emb_dim if ctx else 2*emb_dim, emb_dim, mlplayer, True, **mlp)
...
class SUNConv(Module):
    def __init__(self,
        ...
        self.pool2node = TensorOp.OpPoolingCrossSubg2D(mode[1], pool)
        self.unpool4rootnode = TensorOp.OpUnpoolingRootNodes2D(mode[1], pool)
        self.lin = nn.Sequential(HeteroLinear(7*emb_dim, emb_dim, 2, False), MLP(emb_dim, emb_dim, mlplayer, True, **mlp))
