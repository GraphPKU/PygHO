...
def createemb(dim: int,
              emb_dim: int,
              zeropad: bool = False,
              max_norm: Optional[float] = None,
              orthoinit: bool = False):
    ret = nn.Embedding(dim,
                       emb_dim,
                       max_norm=max_norm,
                       padding_idx=0 if zeropad else None)
    if orthoinit:
...
class SingleEmbedding(nn.Module):
    ...
    def __init__(self,
                 ...
                 dp: float = 0.0) -> None:
        super().__init__()
        self.emb = createemb(dim, emb_dim, lastzeropad > 0, max_norm,
                             orthoinit)
        if ln:
            bn = False
        self.postemb = nn.Sequential()
        if ln:
            self.postemb.append(nn.LayerNorm(emb_dim,
    ...
    def __init__(self,
        ...
        for i, dim in enumerate(dims):
            self.embedding_list.append(
                createemb(dim, emb_dim,
                          len(dims) - i <= lastzeropad, max_norm, orthoinit))
...
class AtomEncoder(nn.Module):
    ...
    def __init__(self, emb_dim, dims=None, lastzeropad=0, orthoinit=False):
        ...
        for i, dim in enumerate(dims):
            emb = nn.Embedding(dim,
                               emb_dim,
                               padding_idx=0 if len(dims) -
                               i <= lastzeropad else None)
            if orthoinit:
