...
class OpNodeMessagePassing(Module):
    def __init__(self, mode: Literal["SS", "SD", "DD"]="SS", aggr: str="sum") -> None:
        super().__init__()
        if mode == "SS":
    ...
    def forward(self, A: Union[SparseTensor, MaskedTensor], X: Union[Tensor, MaskedTensor]) -> Union[Tensor, MaskedTensor]:
        return self.mod.forward(A, X)

class Op2FWL(Module):

    def __init__(self, mode: Literal["SS", "DD"]="SS", aggr: Literal["sum", "mean", "max"]="sum") -> None:
        super().__init__()
        if mode == "SS":
    ...
    def forward(self, X1: Union[SparseTensor, MaskedTensor], X2: Union[SparseTensor, MaskedTensor], datadict: Optional[Dict]=None, tarX: Optional[Union[SparseTensor,MaskedTensor]]=None)->Union[SparseTensor, MaskedTensor]:
        return self.mod.forward(X1, X2, datadict, tarX)


class OpMessagePassingOnSubg2D(Module):

    def __init__(self, mode: Literal["SD", "SS", "DD"]="SS", aggr: Literal["sum", "mean", "max"]="sum") -> None:
        super().__init__()
        if mode == "SS":
    ...
    def forward(self, A: Union[SparseTensor, MaskedTensor], X: Union[SparseTensor, MaskedTensor], datadict: Optional[Dict]=None, tarX: Optional[Union[SparseTensor,MaskedTensor]]=None)->Union[SparseTensor, MaskedTensor]:
...
class OpDiag2D(Module):
    ...
    def forward(self, X: Union[MaskedTensor, SparseTensor]) -> Union[MaskedTensor, Tensor]:
        return self.mod.forward(X)
    
class OpPoolingSubg2D(Module):
    def __init__(self, mode: Literal["S", "D"]="S", pool: str = "sum") -> None:
        super().__init__()
        if mode=="S":
