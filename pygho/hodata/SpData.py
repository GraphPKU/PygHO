...
class SpHoData(PygData):
    ...
    def __inc__(self, key: str, value: Any, *args, **kwargs):
        if key.startswith('tupleid'):
            return getattr(self, "tupleshape"+key.removeprefix("tupleid")).reshape(-1, 1)
        if key.endswith(f"{KEYSEP}acd"):
            key = key.removesuffix(f"{KEYSEP}acd")
            op0, op1, _, op2, _ = parsekey(key)
            return torch.tensor(
                [[getattr(self, parseop(op0))],
                 [getattr(self, parseop(op1))],
...
def sp_datapreprocess(data: PygData, tuplesamplers: Union[Callable[[PygData], Tuple[Tensor, Tensor, Union[List[int], int]]], List[Callable[[PygData], Tuple[Tensor, Tensor, Union[List[int], int]]]]],
                      annotate: List[str]=[""], keys: List[str]=[""]) -> SpHoData:
    data.edge_index, data.edge_attr = coalesce(data.edge_index,
                                               data.edge_attr,
                                               num_nodes=data.num_nodes)
    if not isinstance(tuplesamplers, Iterable):
        tuplesamplers = [tuplesamplers]
    assert len(tuplesamplers) == len(annotate), "number of tuple sampler should match the number of annotate"
    ...
    for i, tuplesampler in enumerate(tuplesamplers):
        ...
        datadict.update({
            f"tupleid{annotate[i]}": tupleid,
            f"tuplefeat{annotate[i]}": tuplefeat,
            f"tupleshape{annotate[i]}": torch.LongTensor(tupleshape).reshape(1, -1),
            f"num_tuples{annotate[i]}": num_tuples
        })
    for key in keys:
        op0, op1, dim1, op2, dim2 = parsekey(key)
        datadict[key+f"{KEYSEP}acd"] = filterind(
            datadict[f"tupleid{op0[1:]}"] if op0[0] == "X" else datadict["edge_index"],
            *spspmm_ind(datadict[f"tupleid{op1[1:]}"] if op1[0] == "X" else datadict["edge_index"],
                        dim1,
                        datadict[f"tupleid{op2[1:]}"] if op2[0] == "X" else datadict["edge_index"],
                        dim2))
