...
def main():
    ...
    parser.add_argument('--an_norm',
                        type=str,
                        default='gcn',
                        choices=["sum", "mean", "max", "gcn"])
    parser.add_argument('--dp', type=float, default=0.9)
    parser.add_argument('--an_dp', type=float, default=0.9)
    parser.add_argument('--num_layer', type=int, default=4)
    parser.add_argument('--an_num_layer', type=int, default=4)
    ...
    parser.add_argument('--jk',
                        ...
                        choices=["sum", "last"],
                        default="last")
    parser.add_argument('--an_jk',
                        type=str,
                        choices=["sum", "last"],
                        default="last")
    parser.add_argument('--res', action="store_true")
    parser.add_argument('--an_res', action="store_true")
    parser.add_argument('--lr', type=float, default=0.0026)
    parser.add_argument('--pool',
                        type=str,
                        choices=["sum", "mean", "max"],
                        default="sum")
    parser.add_argument('--an_set2set',
                        type=str,
                        choices=["id", "mindist", "maxcos", "transformer"],
                        default="id")
    parser.add_argument('--use_elin', action="store_true")
    parser.add_argument('--uni_sample', action="store_true")
    parser.add_argument('--rand_sample', action="store_true")
    parser.add_argument("--mlplayer", type=int, default=1)
    parser.add_argument("--an_mlplayer", type=int, default=1)
    ...
    for rep in range(args.repeat):
        ...
        model = AnchorGNN(dataset.num_tasks,
                          args.num_anchor,
                          args.num_layer,
                          args.emb_dim,
                          args.norm,
                          ...
                          an_num_layer=args.an_num_layer,
                          an_emb_dim=args.emb_dim,
                          an_norm=args.norm,
                          an_virtual_node=False,
                          an_residual=args.an_res,
                          an_dp=args.an_dp,
                          an_JK=args.an_jk,
                          an_set2set=args.an_set2set,
