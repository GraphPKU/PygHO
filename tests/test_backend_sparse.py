...
class SpTensorTest(unittest.TestCase):
    ...
    def test_hash_tight(self):
        ...
        self.assertTrue(tensorequal(dthash, indices), "hash_tight decode wrong")
    ...
    def test_create(self):
        ...
        self.assertTrue(tensorequal(A2.indices,A1c.indices()), "create indice not match")
        self.assertLessEqual(maxdiff(A2.values, A1c.values()), EPS, "create value not match")

        self.assertTrue(tensorequal(A2.indices, A2f.indices), "from coo indice not match")
        self.assertLessEqual(maxdiff(A2.values, A2f.values), EPS, "from coo value not match")

        A1f = A2.to_torch_sparse_coo()
        self.assertLessEqual(maxdiff(A1f.to_dense(), A1c.to_dense()), EPS, "to coo not match")
...
class SpspmmTest(unittest.TestCase):
    ...
    def test_ptr2batch(self):
        ...
        batch = torch.tensor([0,0,0,0,2,2,2,3,4,4,4,7,7,7,7,7], dtype=torch.long)
        self.assertTrue(tensorequal(Spspmm.ptr2batch(ptr, dim_size=16), batch), "ptr2batch error")
    ...
    def test_2dmm(self):
        ...
        out = out.coalesce()
        self.assertTrue(tensorequal(C.indices(), out.indices()), "spspmm indice not match")
        self.assertLessEqual(maxdiff(C.values(), out.values()), EPS, "spspmm value not match")

        tar_ind = torch.stack(
            (torch.randint_like(ind1[0], n), torch.randint_like(ind1[0], l)))

        tar_ind = SpTensor.decodehash(torch.unique(SpTensor.indicehash(tar_ind), sorted=True), 2)
        ...
        self.assertLessEqual(maxdiff(maskedout.to_dense()[tar_ind[0], tar_ind[1]],
                        C.to_dense()[tar_ind[0], tar_ind[1]]), EPS, "spspmm with target ind value not match")
    ...
    def test_2dhadamard(self):
        ...
        Bp = Bp.to_sparse_coo()
        spsphadamardout = Spspmm.spsphadamard(SparseTensor.from_torch_sparse_coo(Ap),
                                    SparseTensor.from_torch_sparse_coo(Bp))
        self.assertLessEqual(
            torch.max(
                (torch.multiply(Ap, Bp) -
                spsphadamardout.to_torch_sparse_coo()).coalesce().values().abs()).item(), EPS, "hadamard error")
    ...
    def test_3dmm(self):
        ...
        self.assertTrue(tensorequal(Cs.indices(), out.indices()), "spspmm indice not match")
        self.assertLessEqual(maxdiff(Cs.values(), out.values()), 1e-5, "spspmm value not match")

        tar_ind = torch.stack(
            (torch.randint_like(ind1[0], n), torch.randint_like(ind1[0], m), torch.randint_like(ind1[0], l), torch.randint_like(ind1[0], n)))

        tar_ind = SpTensor.decodehash(torch.unique(SpTensor.indicehash(tar_ind), sorted=True), 4)
        acd = Spspmm.filterind(tar_ind, ind, bcd)
        mult = val1[acd[1]] * val2[acd[2]]
        outval = scatter_add(mult, acd[0], dim_size=tar_ind.shape[1])
        maskedout = torch.sparse_coo_tensor(tar_ind, outval)
        maskedout = maskedout.coalesce()
        # debug spspmm with target filter
        self.assertLessEqual(maxdiff(maskedout.to_dense()[tar_ind[0], tar_ind[1], tar_ind[2], tar_ind[3]], 
                      C[tar_ind[0], tar_ind[1], tar_ind[2], tar_ind[3]]), 1e-5, "spspmm with target indice value not match")
    ...
    def test_3dhadamard(self):
        ...
        self.assertLessEqual(
            torch.max(
                (torch.multiply(Ap, Bp) -
                spsphadamardout.to_torch_sparse_coo()).coalesce().values().abs()).item(), 1e-5, "3d hadamard value error")
