...
class MaTensorTest(unittest.TestCase):
    ...
    def setUp(self) -> None:
        ...
        mask[1, 1:] = False
        mask[1, :, 1:] = False
        vd = torch.masked.masked_tensor(data,
                                        mask.unsqueeze(-1).expand_as(data))
    ...
    def test_filterinf(self):
        ...
        self.assertTrue(
            tensorequal(MaTensor.filterinf(A),
                        torch.tensor([0, 0, 0, 1, 2, 0, 3])),
            "filter inf error")

    def test_fill(self):
        self.assertTrue(
            floattensorequal(
                self.mt.fill_masked(1024),
                torch.masked_fill(
                    self.data,
                    torch.logical_not(self.mask).unsqueeze(-1).expand_as(
                        self.data), 1024)), "mask fill error")
